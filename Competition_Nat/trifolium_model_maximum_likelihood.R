...
N2.mean<-13.181818; N2.std<-14.972; N2.max<-100
 ...
 bev1 <- function(N1,N2, lambda, aiT,aiA) {lambda / (1+aiT*N1+aiA*N2)} # Beverton-Holt
 
 # simulate data
 datXC = tibble(
   N1 = round(runif(N, 0,N1.max))
   ,N2 = round(runif(N, 0,N2.max))
   ,fitness_sim = bev1(N1,N2, 50, aiT, aiA) # fitness, using BH
 ...
 datW = tibble(
   N1 = round(runif(N, 0,N1.max))
   ,N2 = round(runif(N, 0,N2.max))
   ,fitness_sim = bev1(N1,N2, 75, aiT, aiA) # fitness, using BH
   ,fitness_data = fitness_sim  + rnorm(N,0, 5) # add some noise; adding random amount of error
   ,treatment = "W")
 ...
 datD = tibble(
   N1 = round(runif(N, 0,N1.max))
   ,N2 = round(runif(N, 0,N2.max))
   ,fitness_sim = bev1(N1,N2, 15, aiT, aiA) # fitness, using BH
   ,fitness_data = fitness_sim  + rnorm(N,0, 5) # add some noise; adding random amount of error
   ,treatment = "D")
...
#   ,fitness_data=ifelse(treatment %in% c("D"), fitness_data=fitness_data-5,fitness_data=fitness_data#D add effect of subtract from fitness and add error; rnorm assign treatments))
 ...
 ggplot(dat, aes(x= N2, y=fitness_data)) +
   ...
   stat_smooth(method = "nls",
               ...
               se = FALSE, 
               color='orange')
 #graph curved; fitness of N1 related to pop.size N1
...
## Trifolium
tg <- 0.2 ## germination from Andrew's code in the USDA Climvar repository
...
#for (i in 1:length(treatments)){
  ...
  #m1out <- nlsLM(m1TinA, start=list(lambda=50, aiT = .1, aiA=.01),
...
dat2<-annual_growth_competition(N0=300, lambda=50, aiT=0.1, aiA=0.01, aiAA=0.02, aiTA=0.04, tg=0.2, ag=0.9, period=50){
...
#aiTA = 0.03 #affect of Trifolium vs. Avena 
  ...
  dat2 <- cbind.data.frame(t, Nt_aiT, Nt_aiAA)} # bind together time and pop size vectors
  
ggplot(data = dat2) + geom_line(aes(x = t, y = Nt_aiT), color = "green") + geom_line(aes(x = t, y = Nt_aiAA), color = "orange") +
