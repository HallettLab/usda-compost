...
nmds1 <- as.data.frame(scores(nmds.comp.trait, choices=c(1), display=c("species")))
nmds2 <- as.data.frame(scores(nmds.comp.trait, choices=c(2), display=c("species")))

nmds_dat_trait <- cbind(nmds1, nmds2) %>%
  rownames_to_column(var = "species_id") %>%
  separate(species_id, into = c("ID"), sep = "_")
